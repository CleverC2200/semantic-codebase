...
函数“关键逻辑”按纵向步骤阅读，每步并排展示版本核对后的源码片段和解析，分支与循环通过明确去向跳转。`makeIssue` 已补齐逐步说明；其他函数没有专门说明时保留静态控制流解释。函数上下文固定在右侧并默认展开，不能关闭；可拖动分隔线调整两栏宽度。原始事实、来源标识和解析数据集中在“验收资料”的调试区域，普通阅读不再重复展示。
...
`node scripts/run-python-reader.mjs REPO FILE_LIST_JSON NEW_OUTPUT [PRESENTATION_JSON]` 通过显式 Python 文件白名单重新分析并生成独立阅读包，拒绝覆盖已有输出。它不遍历运行目录、不加载仓库运行配置、不执行被索引模块；没有说明时保留 unknown。Python 输入参数与返回注解来自冻结声明，不代表运行结果。样本范围、源码版本和分析边界记录在输出目录的 `receipt.json`；真实业务样本与本地验收材料不随产品发布。
