...
export function buildReaderData({overlay, definitionsByKey, sourceFiles = [], manifest, structuralGraph, runtime}) {
  ...
  const files = (manifest?.files ?? [...new Map(overlay.evidence.map(e=>[e.file_path,{relative_path:e.file_path,source_digest:e.source_digest}])).values()]).map(f=>{
    ...
    const entry=explanations.files[f.relative_path];
  ...
  const definitions=[...definitionsByKey.values()].map(d=>{
    ...
    const signature=callableSignature(rawType)??(getter&&rawType?{inputs:[],output:rawType,raw:rawType,getter:true}:null);
  ...
  return {...data, runtime: runtime ?? null, runtimeBinding: { valid: runtimeValid, reason: runtimeValid ? null : 'runtime_hash_or_version_mismatch' }, mainlines:bindReaderMainlines(data)};
}

export function renderSemanticPreview(input) {
  const data=JSON.stringify(buildReaderData(input)).replaceAll('<','\\u003c').replaceAll('\u2028','\\u2028').replaceAll('\u2029','\\u2029');
  const css=readFileSync(new URL('./semantic-reader.css',import.meta.url),'utf8');
  const js=readFileSync(new URL('./semantic-reader-query.mjs',import.meta.url),'utf8').replace(/^export /gm,'')+'\n'+readFileSync(new URL('./semantic-reader-comparison.mjs',import.meta.url),'utf8').replace(/^export /gm,'')+'\n'+readFileSync(new URL('./semantic-reader-risk.mjs',import.meta.url),'utf8').replace(/^export /gm,'')+'\n'+readFileSync(new URL('./semantic-reader-capabilities.mjs',import.meta.url),'utf8').replace(/^export /gm,'')+'\n'+readFileSync(new URL('./semantic-reader-graph.mjs',import.meta.url),'utf8').replace(/^export /gm,'')+'\n'+readFileSync(new URL('./semantic-reader-client.js',import.meta.url),'utf8');
  const legacy=renderDiagnosticHtml(input).match(/<main class="wrap">([\s\S]*)<\/main>/)?.[1]??'';
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Zod · 源码阅读工作台</title><style>${css}</style></head><body>
<header class="top"><strong>Semantic Codebase</strong><span class="project">Zod / v3</span><div id="global-search-host"><label class="sr-only" for="global-search">搜索文件、函数、主线</label><input id="global-search" role="combobox" aria-expanded="false" aria-controls="global-results" aria-autocomplete="list" autocomplete="off" placeholder="搜索，或输入：函数名可能影响哪里"><kbd>⌘ K</kbd><div id="global-results" role="listbox" aria-label="全局搜索结果" hidden></div></div><button data-mobile-search class="mobile-search">搜索</button><span class="scope">静态分析 / partial</span><button class="directory-toggle" data-directory-toggle aria-expanded="false">导航</button><button data-open-comparison>变更阅读</button><button id="report">验收资料</button></header>
<div class="workspace"><aside class="sidebar" aria-label="项目导航" data-navigation="source"><nav class="sidebar-modes" aria-label="导航方式"><button data-navigation="business" aria-pressed="false">业务能力</button><button data-navigation="source" aria-pressed="true">源码目录</button></nav><section class="business-navigation"><div id="capability-menu"></div></section><details class="sidebar-section business-navigation" open><summary>项目主线 <small>候选</small></summary><div id="mainline-menu"></div></details><details class="sidebar-section source-navigation" open><summary>源码目录 <small>13 文件</small></summary><p class="tree-root">packages / zod / src / v3</p><p class="tree-legend"><span class="route-dot" aria-hidden="true"></span>相关高亮 · 完整目录保持不变</p><div id="tree"></div><p class="scope-note">仅包含本次冻结分析的文件。</p></details></aside>
