...
import { RepositoryIndexer, PythonTreeSitterAdapter, PythonPyrightEnricher, SqliteSnapshotStore, sha256Bytes, canonicalHash } from '../dist/index.js';
...
if (!repoArg || !manifestArg || !outputArg) throw new Error('Usage: node scripts/run-python-reader.mjs REPO FILE_LIST_JSON NEW_OUTPUT [PRESENTATION_JSON]');
...
const files = names.sort().map(relative_path => {
  if (typeof relative_path !== 'string' || !/^[\w./-]+\.py$/.test(relative_path) || relative_path.split('/').some(p => !p || p === '.' || p === '..') || path.isAbsolute(relative_path)) throw new Error('Invalid Python source path');
  ...
  return { relative_path, language: 'python', source_bytes, source_digest: sha256Bytes(source_bytes) };
});
const git = (...args) => execFileSync('git', ['-C', repo, ...args], { encoding:'utf8' }).trim();
const source = { repository_id: path.basename(repo) + '-python-reader', files };
...
const productFiles = execFileSync('git',['-C',productRoot,'ls-files','--cached','--others','--exclude-standard','src','scripts'],{encoding:'utf8'}).trim().split('\n').sort();
const receipt = {
  ...
  boundary:'Only allowlisted Python files; no repository configuration, installed environment, runtime data or external service. Explanations and mainlines are unverified presentation candidates.',
};
