(() => {
  ...
  const M = createReaderGraphModel(D), defs = M.definitions, functions = M.functions, calls = M.calls;
  ...
  const viewKey = () => S.context === 'comparison' ? 'comparison:' + (comparison.base?.snapshot ?? 'none') + ':' + comparison.target.snapshot : S.context === 'capability' ? 'capability:' + S.capability : S.context === 'mainline' ? 'mainline:' + S.mainline : S.file + ':' + (S.reader ?? 'overview') + ':' + S.mode + (S.mode === 'graph' ? ':' + (preference().internal ?? 'overview') : '');
  ...
  function enterFile(path, key = null, remember = true) {
    if (key) { readFunction(key); return; }
    savePosition();
  ...
  function changeBody(change) {
    ...
    return `<h2>${E(change.after?.qualified_name ?? change.before.qualified_name)}</h2><p class="muted">对应依据：${E(change.matching.kind)}${change.matching.actor ? ' · ' + E(change.matching.actor) + ' · ' + E(change.matching.reason) : ''}</p><h3>本次变化</h3><p>${labels[behavior.classification]}</p>${sensitivity.sensitivity.length ? `<p>长期重要性：${sensitivity.sensitivity.map(x => E(x.label) + (x.status === 'confirmed' ? ' · 已确认' : ' · 待核实')).join('、')}</p>` : ''}${behavior.items.map(item => `<section class="call-detail"><h3>${kinds[item.kind]}</h3><p class="muted">静态事实差异；实际行为仍需验证。</p><strong>之前</strong><pre>${E(JSON.stringify(item.before, null, 2))}</pre><strong>现在</strong><pre>${E(JSON.stringify(item.after, null, 2))}</pre></section>`).join('')}<h3>已知调用影响</h3><p>${review.impact?.directCallers.length ?? 0} 个直接调用者 · ${review.impact?.reached.length ?? 0} 个预算内可达定义；静态关联不等于行为已改变。</p><details><summary>核对调用路径</summary>${(review.impact?.paths ?? []).map(p => `<p>${p.keys.map(k => E(affected.definitions.find(d => d.definition_key === k)?.qualified_name ?? k)).join(' → ')}</p>${p.facts.map(f => `<pre>${E(JSON.stringify(f, null, 2))}</pre>`).join('')}`).join('')}</details><p class="muted">${E(review.impact?.completeness.status ?? 'unknown')} · Coverage ${E(review.impact?.coverage.status ?? 'unknown')}；非调用依赖未覆盖。</p><h3>公共契约依赖 · 继承</h3><p>已知派生定义 ${contract.reached.length} 个 · ${E(contract.completeness.status)}</p>${contract.paths.map(p => `<details><summary>${p.keys.map(k => E(affected.definitions.find(d => d.definition_key === k)?.qualified_name ?? k)).join(' → ')}</summary>${p.relations.map(r => `<p>INHERITS · Snapshot ${E(r.snapshot_id)}</p>${r.evidence_ids.map(id => { const e = affected.evidence[id]; return `<p>${E(e?.file_path)} · 字节 ${e?.span?.start_byte ?? '?'}–${e?.span?.end_byte ?? '?'}</p>`; }).join('')}`).join('')}</details>`).join('')}<p class="muted">只覆盖有证据的 INHERITS，不代表调用顺序；数据库、配置和事件依赖未覆盖。</p><h3>验证记录</h3>${review.verification.map(r => `<p>${E(r.kind)} · ${E(r.status)}<br>${E(r.summary)}<br>产物：${E(r.artifact)}</p>`).join('') || '<p>没有适用于当前比较的验证记录。</p>'}<p class="muted">${review.runtimeObserved ? '存在对应执行观测，不等于完整业务验收。' : '尚无对应真实执行证据。'} 导入记录未在本次阅读中重新执行。</p>${review.gaps.filter(g => g.startsWith('verification_missing:')).map(g => `<p>缺少对应验证：${E(kinds[g.split(':')[1]])}</p>`).join('')}<h3>优先核查</h3><ul>${behavior.checks.map(x => `<li>${E(x)}</li>`).join('')}</ul><h3>之前 · ${E(comparison.base.snapshot)}</h3><p class="breadcrumb">${E(change.before?.file_path ?? '无对应定义')}</p><pre>${E(change.beforeSource ?? '没有可核对的基线源码')}</pre><h3>现在 · ${E(comparison.target.snapshot)}</h3><p class="breadcrumb">${E(change.after?.file_path ?? '无对应定义')}</p><pre>${E(change.afterSource ?? '没有可核对的目标源码')}</pre>${change.after && comparison.target.snapshot === D.snapshot && defs.has(change.after.definition_key) ? `<button data-read-function="${E(change.after.definition_key)}">阅读此函数</button>` : ''}`;
  }
  ...
  function tree() {
    ...
    $('mainline-menu').innerHTML = mainlines.map(m => `<button data-mainline="${E(m.id)}" aria-pressed="${S.context === 'mainline' && S.mainline === m.id}" ${m.available ? '' : 'disabled'}><strong>${E(m.title)}</strong><small>${m.available ? '源码支持 · 说明待验证' : E(m.unavailable)}</small></button>`).join('');
  ...
  function functionView() {
    ...
    return `<article class="function-reading"><h1>${E(d.qualified_name)}</h1><p class="function-purpose">${E(sum.text)}</p><p class="muted">${d.line ? 'L' + d.line + '–' + d.endLine : '源码版本未核对'} · ${sum.inferred ? 'AI 说明 · 未验证' : '静态事实摘要 · 功能待补齐'}</p>${importanceHtml(d.definition_key)}${ioHtml(d, true)}${!d.presentation?.logic && blocks.length ? `<h3>处理逻辑 · 静态语句</h3>${blocks.map(b => `<pre>${E(b.source_excerpt ?? b.kind)}</pre>`).join('')}` : ''}${dataFlowHtml(d.definition_key)}<details class="explanation-origin"><summary>解释来源与完整声明</summary><p>功能、参数含义及示例如有 AI 说明，属于 llm_inferred、未验证的展示内容，绑定当前源码哈希；类型来自已有编译器事实。分析覆盖仍为 partial。</p>${d.rawType ? `<pre>${E(d.rawType)}</pre>` : ''}</details></article>`;
  }
  ...
  function currentGraph() {
    ...
    const key = S.context === 'mainline' ? 'mainline:' + S.mainline : S.file + ':' + S.mode + ':' + (S.mode === 'calls' ? S.reader : pref.internal ?? 'file') + ':' + pref.expanded.join(',');
    ...
    } else graph = layoutReaderGraph(S.mode === 'calls' && S.reader ? M.functionCalls(S.reader) : pref.internal ? M.logicGraph(pref.internal) : M.fileGraph(S.file, pref.expanded));
    return graph;
  }
  ...
  function graphHtml() {
    ...
    if (graph.compact) return `<section class="key-logic" aria-label="关键逻辑步骤"><p class="muted">${graph.nodes.length} 个关键步骤 · 静态路径；选择步骤核对源码。</p><ol>${graph.nodes.map(n => `<li data-node-id="${E(n.id)}"><button data-graph-node="${E(n.id)}"><strong>${E(n.title)}</strong></button><p>${E(n.description)}</p>${n.inferred ? '<small>AI 说明 · 未验证</small>' : ''}</li>`).join('')}</ol>${graph.unknowns?.length ? `<details class="explanation-origin"><summary>分析缺口 · ${graph.unknowns.length}</summary><p>${graph.unknowns.map(E).join('、')}</p></details>` : ''}</section>`;
  ...
  function inspector() {
    ...
    if (S.detail === 'fn') {
      ...
      body = `${S.reader === S.fn ? '' : `<h2>${E(title)}</h2><p class="function-purpose">${E(sum.text)}</p><button data-read-function="${E(d.definition_key)}">阅读此函数</button>`}<p class="breadcrumb">${E(d.file_path)} · ${d.line ? 'L' + d.line : '源码版本未核对'}</p><div class="section-title" id="detail-source"><h3>对照源码</h3><button data-wrap aria-pressed="${wrapSource}">自动换行：${wrapSource ? '开' : '关'}</button></div>${sourceHtml(file, d)}<section id="detail-relations"><h3>关联</h3><details class="details"><summary>参与的项目主线 · ${related.length}</summary><div class="links">${related.map(m => `<button data-mainline="${E(m.id)}">${E(m.title)}</button>`).join('') || '尚未整理相关主线。'}</div></details><details class="details"><summary>谁调用它 · ${new Set(calls.filter(f => f.value.target_definition_key === S.fn && f.subject.definition_key !== S.fn).map(f => f.subject.definition_key)).size} 个调用者 · ${calls.filter(f => f.value.target_definition_key === S.fn).length} 处位置</summary>${callsHtml(calls.filter(f => f.value.target_definition_key === S.fn), 'incoming')}</details><details class="details"><summary>它调用谁 · ${calls.filter(f => f.subject.definition_key === S.fn).length} 处位置</summary>${callsHtml(calls.filter(f => f.subject.definition_key === S.fn), 'outgoing')}</details><details class="details"><summary>内部条件与副作用</summary>${cfg ? cfg.value.blocks.filter(b => !['entry', 'exit'].includes(b.kind)).map(b => `<pre>${E(b.source_excerpt ?? b.kind)}</pre>`).join('') : '<p>内部控制流未提取，可直接阅读源码。</p>'}${own(S.fn).filter(f => f.kind === 'effect').map(f => `<p>可能的副作用：${E(f.value.operation ?? f.value.effect_kind)}</p>${factProof(f)}`).join('')}</details></section><section id="detail-evidence"><h3>证据与来源</h3><p class="muted">静态分析事实，不代表实际运行；覆盖仍为 partial。</p>${own(S.fn).map(factProof).join('') || '<p>本次没有可展示的函数事实。</p>'}</section>`;
  ...
  function render(content = true) {
    ...
    if (content) {
      ...
      $('center').innerHTML = S.context === 'comparison' ? comparisonView() : S.context === 'capability' ? capabilityView() : S.context === 'mainline' ? mainlineView() : S.mode === 'text' ? (S.reader ? functionView() : fileView()) : fileGraphView();
    }
  ...
  function searchGlobal(query = '') {
    const q = query.trim().toLowerCase(); globalIndex = -1; globalMatches = [];
    const answer = queryReader(D, query, { capabilities, risk });
    if (answer.intent || /[？?]|哪里|为何|为什么/.test(query)) {
      globalMatches = answer.objects;
      const messages = { answered: '已定位，选择对象下钻核对', ambiguous: '存在同名对象，请按文件或对象类型消歧', no_match: '没有精确匹配，请使用完整函数名或能力／主线标题', unsupported: '支持：某功能从哪里进入；某函数可能影响哪里' };
      $('global-results').innerHTML = `<p class="search-hint" role="status">${messages[answer.status]}</p><p class="search-hint">Snapshot ${E(answer.snapshot)} · 离线冻结范围 · Coverage partial</p>${answer.objects.map((o, i) => `<button role="option" aria-selected="false" tabindex="-1" id="global-option-${i}" data-global-pick="${i}"><strong>${E(o.title)}</strong><small>${E(o.subtitle ?? ({capability:'能力候选',mainline:'主线候选',fn:'函数'}[o.type]))}</small></button>`).join('')}<p class="search-hint">${answer.intent === 'impact' ? '下钻函数查看有界影响、路径证据及未知项。' : '能力归属与主线入口为候选，可下钻核对源码。'}</p>`;
      $('global-results').hidden = false; $('global-search').setAttribute('aria-expanded', 'true'); return;
    }
    const groups = [
      { name: '文件', items: D.files.filter(f => f.path.toLowerCase().includes(q)).map(f => ({ type: 'file', key: f.path, title: f.path.split('/').pop(), subtitle: f.path })) },
      { name: '函数', items: functions.filter(d => `${d.qualified_name} ${d.file_path}`.toLowerCase().includes(q)).map(d => ({ type: 'fn', key: d.definition_key, title: d.qualified_name, subtitle: d.file_path })) },
      { name: '项目主线', items: mainlines.filter(m => m.available && `${m.title} ${m.purpose}`.toLowerCase().includes(q)).map(m => ({ type: 'mainline', key: m.id, title: m.title, subtitle: m.purpose })) },
    ];
    const total = groups.reduce((n, g) => n + g.items.length, 0);
    $('global-results').innerHTML = `<p class="search-hint" role="status">${q ? total + ' 个结果' : '输入关键词定位；目录不会被搜索过滤'}</p>` + groups.filter(g => g.items.length).map(group => `<div role="group" aria-label="${group.name}"><p class="search-group">${group.name} · ${group.items.length}${group.items.length > 8 ? '（显示前 8 项）' : ''}</p>${group.items.slice(0, 8).map(item => { const i = globalMatches.push(item) - 1; return `<button role="option" aria-selected="false" tabindex="-1" id="global-option-${i}" data-global-pick="${i}"><strong>${E(item.title)}</strong><small>${E(item.subtitle)}</small></button>`; }).join('')}</div>`).join('') + (total ? '' : '<p class="empty">没有匹配结果，请尝试函数名或文件路径。</p>');
    $('global-results').hidden = false; $('global-search').setAttribute('aria-expanded', 'true'); $('global-search').removeAttribute('aria-activedescendant');
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (el.hasAttribute('data-open-comparison')) enterComparison();
    else if (el.hasAttribute('data-export-bundle')) { const url = URL.createObjectURL(new Blob([JSON.stringify(D)], { type: 'application/json' })); const link = document.createElement('a'); link.href = url; link.download = 'reader-' + D.snapshot.slice(0, 16) + '.json'; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
    ...
    else if (el.dataset.mainline) enterMainline(el.dataset.mainline, Boolean(el.closest('#inspector')) || S.context === 'capability');
